...
test('loads config and advances from setup into the recorder path', async ({ page }) => {
  ...
  const localModeSummary = page.getByLabel('Local mode summary')
  await expect(localModeSummary.getByText('No account')).toBeVisible()
  await expect(localModeSummary.getByText('D-drive library')).toBeVisible()
  await expect(localModeSummary.getByText('Private until shared')).toBeVisible()
  ...
  await expect(storageHealth.getByText('Guardrail: 2.0 GB per recording.')).toBeVisible()
  ...
  await expect(preflight.getByText('Limit')).toBeVisible()
  await expect(preflight.getByText('2.0 GB max')).toBeVisible()
...
test('guides first-run from record draft to save then share', async ({ page }) => {
  ...
  await expect(draftStatus.getByText(/\d+ KB draft/)).toBeVisible()
  ...
  await expect(page.getByText('No recordings yet')).toBeHidden()
  await expect(page.getByLabel('Current guidance').getByText('Copy the link')).toBeVisible()
  ...
  await expect(readyState.getByText('No account needed. Copy the guest link')).toBeVisible()
  await expect(readyState.getByText('Preview before sending')).toBeVisible()
  await expect(readyState.getByText('check playback before sending')).toBeVisible()
  await expect(shareDialog.getByRole('button', { name: 'Copy guest link' })).toBeVisible()
...
test('applies password expiry and playback-only share settings for guests', async ({
  ...
  await expect(shareDialog.getByLabel('Share ready').getByText('Ready to send')).toBeVisible()
  await expect(shareDialog.locator('.share-state').getByText('Viewer password')).toBeVisible()
...
test('keeps the guided path usable on a mobile viewport', async ({ page, request }) => {
  ...
  await expect(page.getByRole('heading', { name: 'Ready Room' })).toBeVisible()
  const mobileLocalModeSummary = page.getByLabel('Local mode summary')
  await expect(mobileLocalModeSummary.getByText('No account')).toBeVisible()
  await expect(mobileLocalModeSummary.getByText('D-drive library')).toBeVisible()
  await expect(page.getByLabel('Current guidance').getByText('Confirm the room')).toBeVisible()
